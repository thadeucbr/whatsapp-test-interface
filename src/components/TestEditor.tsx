  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-4">
      ...
      <div className="space-y-6 max-h-[calc(100vh-12rem)] overflow-y-auto">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Nome do Caso de Teste</label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            placeholder="Digite o nome do caso de teste"
          />
        </div>
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-md font-medium text-gray-800">Interações</h3>
            <button
          ...
          {interactions.map((interaction, index) => (
            <div key={index} className="space-y-4 p-4 bg-white border rounded-lg">
              <div className="flex justify-between items-center">
                <h4 className="font-medium">Interação {index + 1}</h4>
                <button
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Mensagem do Usuário</label>
                <textarea
